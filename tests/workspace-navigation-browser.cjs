...
(async()=>{
 ...
 try{
  for(const width of [1440,390]){
   ...
   await context.addInitScript(({raw})=>{if(!sessionStorage.getItem('navigation-seeded')){sessionStorage.setItem('navigation-seeded','1');localStorage.setItem('morning-launchpad-summary:v1',raw);}}, {raw:JSON.stringify({version:2,items:[note],briefing:''})});
   ...
   assert.deepEqual(errors,[]);await context.close();console.log(`PASS ${width}: saved navigation, draft protection, close warning and data preservation`);
  }
