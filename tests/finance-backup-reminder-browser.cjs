...
(async()=>{
  ...
  try{
    ...
    let download=page.waitForEvent('download');await page.locator('#downloadFinanceCopy').click();await download;await page.locator('#confirmFinanceBackup').waitFor({state:'visible'});assert.equal(await unload(),true);
