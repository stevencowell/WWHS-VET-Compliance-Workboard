...
(async()=>{
  ...
  try{
    ...
    await openSave(page);await page.locator('#financeBackupSettings > summary').click();
    ...
    await edit(200);const before=await page.evaluate(k=>window.FINANCE_STORAGE.getItem(k),META);await page.evaluate(()=>window.__pickerMode='cancel');await openSave(page);await page.locator('#downloadEncryptedBackup').click();await page.waitForFunction(()=>!document.querySelector('#downloadEncryptedBackup').disabled);assert.equal(await page.evaluate(k=>window.FINANCE_STORAGE.getItem(k),META),before);assert.equal(await unload(),true);assert.match(await page.locator('#backupStatus').innerText(),/Save cancelled/);
    await page.evaluate(()=>window.__pickerMode='write-fail');await openSave(page);await page.locator('#downloadEncryptedBackup').click();await page.waitForFunction(()=>document.querySelector('#backupStatus').textContent.includes('could not be saved'));assert.equal(await unload(),true);
    await page.evaluate(()=>window.__pickerMode='blocked');let download=page.waitForEvent('download');await openSave(page);await page.locator('#downloadEncryptedBackup').click();await download;await page.locator('#confirmFinanceBackup').waitFor({state:'visible'});assert.match(await page.locator('#backupStatus').innerText(),/download started/);assert.equal(await unload(),true);
    ...
    await edit(400);await page.evaluate(()=>window.__pickerMode='defer');await openSave(page);await page.locator('#downloadEncryptedBackup').click();await page.waitForFunction(()=>typeof window.__releaseFile==='function');await edit(500);await page.evaluate(()=>window.__releaseFile());await page.waitForFunction(()=>document.querySelector('#backupStatus').textContent.includes('more changes arrived'));assert.equal(await unload(),true);assert.equal(await page.locator('#confirmFinanceBackup').isVisible(),false);
    let lockAsked=false;page.once('dialog',async d=>{lockAsked=d.message().includes('Lock Finance anyway');await d.dismiss();});await closeSave(page);await page.locator('#lockFinance').click();assert.equal(lockAsked,true);assert.equal(await page.locator('#financeWorkspace').isVisible(),true);
    await openSave(page);await page.locator('#financeBackupAlerts').uncheck();await page.waitForFunction(()=>document.querySelector('#saveStatus').textContent==='Saved in this browser');assert.equal(await unload(),false);
    ...
    await closeSave(page);await page.locator('#lockFinance').click();await page.locator('#vaultPassword').waitFor({state:'visible'});await page.locator('#vaultPassword').fill('synthetic private test password');await page.locator('#unlockFinance').click();await page.waitForFunction(()=>document.querySelector('#saveStatus').textContent==='Saved in this browser');assert.equal(await page.locator('#financeBackupAlerts').isChecked(),false);
    await openSave(page);await page.locator('#financeBackupSettings > summary').click();
