...
const base=process.env.TEST_BASE_URL||'http://127.0.0.1:4173',key='morning-launchpad-summary:v1',outputs=path.resolve(__dirname,'../../../outputs');
...
async function saved(p){return p.evaluate(k=>JSON.parse((window.WWHS_STORAGE||localStorage).getItem(k)),key);}
...
(async()=>{
  ...
  await check('all active native VET and TAS task contexts validate and offer specific preparation',async()=>{
    const {c,p}=await setup();try{for(const route of ['/#my-work','/head-teacher-tas/#my-work']){
      ...
      });assert.ok(result.length>=41);assert.ok(result.every(r=>r.profile!=='source-task-preparation'),JSON.stringify(result.filter(r=>r.profile==='source-task-preparation')));
  ...
  await check('visible edits and subsequent saved changes produce fresh requests; notes require opt-in',async()=>{
    const {c,p}=await setup();try{await go(p,'/head-teacher-tas/#my-work');const item=(await saved(p)).items.find(i=>i.forecast?.section==='ready');
      ...
      await p.getByRole('checkbox',{name:'Include my note',exact:true}).check();assert.ok((await prompt(p)).includes('PRIVATE_WORKING_NOTE_739'));
      ...
      await close(p);await open(p,item.id);assert.equal(await p.getByRole('checkbox',{name:'Include my note',exact:true}).isChecked(),false);
    }finally{await c.close();}
  });
  await check('native private exception notes and overridden links are excluded from requests',async()=>{
    const {c,p}=await setup();try{await go(p,'/head-teacher-tas/#my-work');const task=(await saved(p)).items.find(i=>i.origin.recordKey.includes('2026-09-14'));
      await p.evaluate(({recordKey})=>{const k=window.HT_TAS_WORKBOARD.config.storageKey;const s=JSON.parse((window.WWHS_STORAGE||localStorage).getItem(k)||'{}');s.schemaVersion=2;s.linkDefaultsVersion=2;s.records={...(s.records||{}),[recordKey]:{status:'waiting',steps:{},milestones:{},exceptionReason:'PRIVATE_NATIVE_EXCEPTION_520'}};s.links={sentral:'https://example.org/PRIVATE_OVERRIDE_900'};(window.WWHS_STORAGE||localStorage).setItem(k,JSON.stringify(s));},task.origin);
  ...
  await check('source step refresh is independent of personally edited next action',async()=>{
    const {c,p}=await setup();try{await go(p,'/#my-work');const item=(await saved(p)).items.find(i=>i.origin.taskId==='a-01-confirm-authority-set');
      ...
      await p.evaluate(id=>{const k=window.VET_WORKBOARD.config.storageKey;const s=JSON.parse((window.WWHS_STORAGE||localStorage).getItem(k)||'{}');s.schemaVersion=3;s.records={...(s.records||{}),[id]:{status:'in-progress',stepChecks:{0:true},history:[]}};(window.WWHS_STORAGE||localStorage).setItem(k,JSON.stringify(s));},item.origin.taskId);
  ...
  await check('saved native help remains available in Launchpad and another work area',async()=>{
    const {c,p}=await setup();try{await go(p,'/#my-work');const item=(await saved(p)).items[0];
      for(const route of ['/morning-launchpad/','/head-teacher-tas/#my-work']){await go(p,route);await open(p,item.id);const text=await prompt(p);assert.ok(text.includes(item.origin.recordKey));assert.match(text,/saved VET task/);await close(p);}
    }finally{await c.close();}
  });
  await check('weekly occurrences stay separate and old source progress is not assumed current',async()=>{
    const {c,p}=await setup();try{await go(p,'/head-teacher-tas/#my-work');const first=(await saved(p)).items.find(i=>i.origin.recordKey.includes('2026-09-14'));
      ...
      await open(p,first.id);const oldText=await prompt(p);assert.ok(oldText.includes(first.origin.recordKey));assert.ok(!oldText.includes(next.origin.recordKey));assert.match(oldText,/saved copy of the task/);await close(p);
  ...
  await check('unavailable native source warns honestly and conflicting shared work blocks copying',async()=>{
    const {c,p}=await setup();try{await go(p,'/#my-work');const item=(await saved(p)).items[0];await p.evaluate(()=>localStorage.setItem(window.VET_WORKBOARD.config.storageKey,'{broken'));await open(p,item.id);
      assert.match(await prompt(p),/CURRENT SOURCE CHECK UNAVAILABLE/);assert.match(await p.locator('.task-help-state').innerText(),/saved copy of the task/);await close(p);
      await p.evaluate(k=>(window.WWHS_STORAGE||localStorage).setItem(k,(window.WWHS_STORAGE||localStorage).getItem(k)+' '),key);await open(p,item.id);assert.equal(await prompt(p),'');assert.ok(await p.getByRole('button',{name:'Copy help request',exact:true}).isDisabled());
    }finally{await c.close();}
  });
