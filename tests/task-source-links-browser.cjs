...
(async()=>{
  ...
  try{
    ...
    console.log('PASS VET: visible exact handbook source, source before steps, exact audit focus/reset, unchanged native state, mobile overflow');
    ...
    for(const fixture of [
      ...
      assert.ok(await panel.evaluate(node=>Boolean(node.compareDocumentPosition(document.querySelector('#task-dialog .action-list'))&Node.DOCUMENT_POSITION_FOLLOWING)));
      ...
      await page.setViewportSize({width:1440,height:1100});
      console.log('PASS TAS '+fixture.label+': exact source visible before steps, mobile overflow');
    }
