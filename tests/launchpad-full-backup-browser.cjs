...
(async()=>{
 ...
 try{
  for(const width of [1440,390]){
   ...
   dialog=await open(page,JSON.stringify(newer));const choice=dialog.getByRole('checkbox',{name:'Use the backup’s version when an item is different'});assert.equal(await choice.isVisible(),true);assert.equal(await choice.isChecked(),false);await apply(dialog);assert.deepEqual(await snapshot(page),first);
   dialog=await open(page,JSON.stringify(newer));await dialog.getByRole('checkbox',{name:'Use the backup’s version when an item is different'}).check();await dialog.getByRole('button',{name:'Cancel',exact:true}).click();assert.deepEqual(await snapshot(page),first);
   dialog=await open(page,JSON.stringify(newer));await dialog.getByRole('checkbox',{name:'Use the backup’s version when an item is different'}).check();await apply(dialog);
