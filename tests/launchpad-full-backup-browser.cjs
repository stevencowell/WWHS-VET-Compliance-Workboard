...
(async()=>{
 ...
 try{
  for(const width of [1440,390]){
   ...
   await page.getByText('More backup options',{exact:true}).click();const previous=page.getByRole('button',{name:'Save previous copy…',exact:true});await previous.waitFor({state:'visible'});
   ...
   const pending=page.waitForEvent('download');await page.locator('[data-backup-action="save"]').click();const download=await pending;assert.match(download.suggestedFilename(),/^launchpad-backup.*\.json$/);const saved=await fs.readFile(await download.path(),'utf8');
