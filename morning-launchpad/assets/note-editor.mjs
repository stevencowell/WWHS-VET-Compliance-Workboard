...
export function createNoteEditor(item,{disabled,save}){
  ...
  function persist(){clearTimeout(timer);if(!dirty)return;const text=editor.innerText;const html=cleanNoteHtml(editor.innerHTML);if(text.length>200000||html.length>1000000){status.textContent='Not saved: shorten this note to 200,000 characters.';return;}if(save({noteText:text,noteHtml:html})){dirty=false;status.textContent='Saved';}else status.textContent='Not saved — see the message above.';}
  function changed(){dirty=true;status.textContent='Saving…';clearTimeout(timer);timer=setTimeout(persist,500);}
