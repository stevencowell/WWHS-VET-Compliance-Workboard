import {enrich,linksIn,todaySydney,validDate,SOURCE_LIMIT} from './summary-core.mjs?v=vet-admin-procedures-20260924';
...
export function suggestEmail(source,{subject='',instruction='',today=todaySydney()}={}){
 ...
 return enrich({id:'email-preview',title,action,source:text,noteText:text,links:linksIn(text),url:'',instruction:direct,group,priority,nextAction:waiting?'delay':deadline.date?'date':actionable?'do':'delay',dueDate:deadline.date,eventDate:eventDate.date,reason:`Suggested from the email wording: ${reason}.`,dateNote,score:urgent?70:actionable?30:0});
}
