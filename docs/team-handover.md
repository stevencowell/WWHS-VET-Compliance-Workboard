...
- Import retains the previous portable team snapshot locally. The **Download recovery copy** action exports that snapshot with a new workspace identity for review, excluding private fields. One pre-import recovery is retained; archive external versions for longer history. Personal email content is not duplicated into the retained team recovery snapshot.
