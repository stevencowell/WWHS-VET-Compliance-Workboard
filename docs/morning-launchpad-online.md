...
Imported Sentral teaching lessons and duties use a blue background and bold text; diary and other imported events use muted grey. Existing timetable entries are recognised from their class/duty code and Period description, so no reimport or data migration is needed. Personal dates stay green, and derived task dates retain their priority colours. The legend, tooltips and event dialog also identify the category in text.
...
The downloadable AI instructions now include the revised full master prompt. Every actionable task is assessed for concrete AI assistance; only the final starting shortlist is capped at five. Existing saved help text is not automatically regenerated. Reprocess the export with the current task backup to refresh help while retaining identities and progress. The standalone master prompt is also downloadable from the import panel.
