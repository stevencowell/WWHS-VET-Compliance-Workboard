...
async function saveMeta(before,next){
  // Recovery exports use the shared snapshot. Older releases also saved a redundant
  // raw copy of the whole inbox (including private mail), which no action reads.
  if(next.recovery){const {raw,...recovery}=next.recovery;next={...next,recovery};}
  await applyTeamTransaction(localStorage,before,{...before,[KEYS.metadata]:JSON.stringify(next)});
...
async function importFile(editing){
  ...
  await applyTeamTransaction(localStorage,{...plan.before,[KEYS.metadata]:before[KEYS.metadata]},{...plan.after,[KEYS.metadata]:JSON.stringify(next)});
...
const initialControls=[...document.querySelectorAll('button,input,textarea')];
...
try{
  ...
  const recovered=await recoverTeamTransaction(localStorage);initialControls.forEach(control=>control.disabled=false);render();if(recovered.status!=='none')say('The interrupted handover has been recovered. Check the current status before continuing.');
