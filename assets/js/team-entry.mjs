...
export function installTeamEntry({wing,base,header}) {
  ...
  const importLink=make('a','Import backup…',{href:`${handover}#import-backup`,class:'team-entry-primary'});
  ...
  copy.append(make('span','!',{class:'team-entry-mark','aria-hidden':'true'}),make('span','VET + TAS · Shared progress',{class:'team-entry-label'}),make('strong','Import & save'),heading,description,make('span','WWHS-team-handover.json · One file for both sections',{class:'team-entry-file'}));actions.append(importLink,saveLink,guide);banner.append(copy,actions);header.after(banner);
  ...
  const second=make('li','Choose Import backup… below and select the latest file from your synced Google Drive folder, or the copy downloaded to this device. Preview it, then choose Import to view only or start an editing session.');steps.append(first,second);
  const primary=make('a','Import backup…',{href:`${handover}#import-backup`,class:'team-entry-primary',autofocus:''});
  ...
  function refresh(message){
    ...
    importLink.title=state?.active?'Finish the current handover before importing another backup':'Import the latest shared VET and TAS backup';
    saveLink.title=editing?'Save your shared changes and finish the handover':'Open the shared backup and handover controls';
