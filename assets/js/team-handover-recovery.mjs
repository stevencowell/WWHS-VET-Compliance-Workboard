...
import {KEYS,metadataKey,scopeSnapshot,readRaw,snapshot,parseBackup,createBackup,buildImportPlan} from './team-handover-core.mjs?v=edit-card-text-1';
import {archiveTeamState,prepareTeamMetadata} from './team-handover-payloads.mjs?v=edit-card-text-1';
...
const info=payload=>Object.fromEntries(INFO.filter(key=>payload[key]!==undefined).map(key=>[key,payload[key]]));
...
export function parseTeamFile(input){
  ...
  if(!object(value)||value.schemaVersion!==1||Object.keys(value).some(key=>!['scope',...SAFETY_FIELDS].includes(key))||SAFETY_FIELDS.some(key=>!Object.hasOwn(value,key)))fail('The safety backup format is invalid. Nothing has been replaced.');
  // Reuse every shared-data and identity check without adding revision or
  // ancestry claims to the actual portable safety file.
  const checked=parseBackup({...(value.scope?{scope:value.scope}:{}),kind:'WWHS-TEAM-HANDOVER',schemaVersion:1,workspaceId:'safety-validation',revision:1,
  ...
  const file={...(checked.scope?{scope:checked.scope}:{}),kind:SAFETY_KIND,schemaVersion:1,snapshotId:checked.exportId,savedAt:checked.savedAt,savedBy:checked.savedBy,note:checked.note,data:checked.data};
  return {type:'safety',file,data:file.data};
}
export function createSafetyBackup({snapshot:data,editor,note='',savedAt=new Date().toISOString(),snapshotId=id(),scope=null}){
  return parseTeamFile({...(scope?{scope}:{}),kind:SAFETY_KIND,schemaVersion:1,snapshotId,savedAt,savedBy:editor,note,data:scopeSnapshot(data,scope)}).file;
}
...
export function buildReconnectPlan(storage,input,{editor,editing=true,allowUnverifiedLineage=false,sessionId=id(),startedAt=new Date().toISOString(),workspaceId=id(),exportId=id(),createId=id,scope=null}={}){
  const source=parseTeamFile(input);
  if(scope&&source.file.scope&&scope!==source.file.scope)fail(`This is a ${source.file.scope.toUpperCase()} backup. Open it in that area; nothing has changed.`);
  scope=scope||source.file.scope||null;const metaKey=metadataKey(scope);
  const before={...readRaw(storage),[metaKey]:storage.getItem(metaKey)};
  const legacy=scope&&before[metaKey]===null;
  if(legacy)before[KEYS.metadata]=storage.getItem(KEYS.metadata);
  const connection=savedConnection(legacy?before[KEYS.metadata]:before[metaKey]);
  ...
  if(source.type==='safety'){
    payload=createBackup({scope,snapshot:source.data,editor:editor||source.file.savedBy,workspaceId,exportId,savedAt:startedAt,
  ...
  const plan=buildImportPlan(captured,payload,{lastFile:source.type==='safety'?null:connection.lastFile,
    firstConnection:source.type==='safety'||!connection.lastFile,createId,scope});
  ...
  const nextMetadata={version:1,lastFile:info(payload),
    active:editing?{id:sessionId,editor,startedAt,phase:'editing',baseExportId:payload.exportId,baselineData:scopeSnapshot(source.data,scope)}:null,
  ...
  return {...plan,before,beforeSnapshot,metadataKey:metaKey,scope,connectionRaw:legacy?before[KEYS.metadata]:before[metaKey],nextMetadata,sourceType:source.type,sourceFile:source.file,lineage,
...
export async function prepareReconnectPlan(input,options={}){
  ...
  if(!object(plan.before)||!Object.hasOwn(plan.before,plan.metadataKey||KEYS.metadata)||!object(plan.nextMetadata))fail('The reconnect preview is invalid. Choose the file again.');
  const archiveRef=await archiveTeamState({metadataRaw:Object.hasOwn(plan,'connectionRaw')?plan.connectionRaw:plan.before[plan.metadataKey||KEYS.metadata],data:plan.beforeSnapshot,
  ...
  const after={...plan.before,...plan.after,[plan.metadataKey||KEYS.metadata]:JSON.stringify(nextMetadata)};
