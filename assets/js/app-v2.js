(function () {
  ...
  function headTeacherGuidePanel() {
    ...
    return `<details class="reference-map"><summary><span>${esc(guide.title)}</span><small>${guide.sections.length} sections mapped · opens only when needed</small></summary><div class="reference-map-body"><div class="reference-map-intro"><div><p>${esc(guide.description)}</p><small><strong>Safe launch:</strong> choose the education account, then use the exact Drive search result. The direct document link is not published and the sharing review remains open.</small></div><button class="button ${ready ? "secondary" : "quiet"}" type="button" data-action="open-system" data-system-id="head-teacher-guide">${ready ? "Find in school Drive ↗" : "Set private staff link"}</button></div><div class="reference-section-list">${guide.sections.map(section => `<article class="reference-section-row"><span class="reference-section-code">${esc(section.code)}</span><div><h3>${esc(section.title)}</h3><p>${esc(section.covered)}</p><small><strong>Keep in the reference guide:</strong> ${esc(section.keep)}</small><nav aria-label="${esc(section.title)} workboard destinations">${section.links.map(([label, href]) => `<a href="${esc(href)}">${esc(label)}</a>`).join("")}</nav></div></article>`).join("")}</div></div></details>`;
  }
