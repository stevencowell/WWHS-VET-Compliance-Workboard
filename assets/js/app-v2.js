(function () {
  ...
  const defaultState = {
    ...
    links: {}, records: {}, statuses: {}, assignments: {}, weekly: {}, gaps: {}, resetArmed: false, lastBackup: ""
  };
  let state = loadState();
  ...
  function renderSystems() {
    ...
    route.innerHTML = `<section class="page"><header class="page-heading"><div><p class="eyebrow">Open the right place first</p><h1>Systems &amp; sources</h1><p>Plain-language launch points for the authorised systems that own the work. Private links are never embedded in the rebuild; set them locally once.</p></div><button class="button" type="button" data-action="open-settings">Set approved links</button></header><aside class="notice compact-notice"><span class="notice-icon" aria-hidden="true">i</span><div><strong>Signed-in work account required</strong><p>Before acting, confirm the authorised education account and the current source version. A familiar saved link can still be stale.</p></div></aside>${groups.map(group => `<section class="system-group"><div class="section-heading"><div><h2>${esc(group)}</h2></div></div><div class="system-cards">${data.systems.filter(system => system.group === group).map(systemCard).join("")}</div></section>`).join("")}<details class="source-library"><summary>Open the authority and source register <span>${data.sources.length} mapped sources</span></summary><div class="source-list">${data.sources.map(sourceCard).join("")}</div></details></section>`;
  }
  ...
  function sourceCard(source) { return `<article class="source-card"><div><span class="source-access">${esc(source.access)}</span><h3>${esc(source.title)}</h3><p>${esc(source.owner)} · checked ${esc(source.verified)}</p><small>${esc(source.note)}</small></div>${source.url ? `<a href="${esc(source.url)}" target="_blank" rel="noopener">Open current source ↗</a>` : `<span class="controlled-label">Open through approved staff system</span>`}</article>`; }
  ...
  function saveSettings(form) {
    ...
    data.systems.filter(system => !system.url).forEach(system => { const proposed = String(values.get(`link:${system.id}`) || "").trim(); if (!proposed) delete state.links[system.id]; else if (safeUrl(proposed)) state.links[system.id] = proposed; });
    saveState(); roleFilter.value = state.role; updateGuidanceToggle(); settingsDialog.close(); render(); toast("Workspace setup saved in this browser");
  }
