(function (root) {
  ...
  const scopeKey=scope=>`wwhs-team-handover:${scope}:v1`;
  const defaultScope=()=>root.document?.documentElement?.dataset?.workspace==='tas'?'tas':'vet';
  function read(scope=defaultScope()) {
    try {const raw=browserStorage().getItem(scopeKey(scope))??browserStorage().getItem(KEY);if(!raw)return {managed:false};const state=JSON.parse(raw);if(state?.version!==1||!state.lastFile?.workspaceId||!Number.isInteger(state.lastFile.revision))throw Error();return {...state,managed:true};}
  ...
  function operational(key,value,scope) {
    const state=typeof value==='string'?JSON.parse(value):value||{};
    if(key===VET&&scope==='tas'||key===TAS&&scope==='vet')return null;
    ...
    if(key===REVIEW)return Object.fromEntries(Object.entries(state.records||{}).filter(([id])=>!scope||id.startsWith(`${scope}:`)));
    if(key===INBOX)return {items:(state.items||[]).filter(item=>isTeamItem(item)&&(!scope||item.origin.wing===scope)).map(item=>{
      ...
      return shared;
    }).sort((a,b)=>a.taskKey.localeCompare(b.taskKey)),workboardImports:(state.workboardImports||[]).filter(id=>!scope||id.startsWith(`${scope}:`))};
    return null;
  }
  ...
  function allowWrite(key,next) {
    ...
    // slice independently so a view-only VET copy cannot block TAS work.
    if(!hasJournal()){
      try{
        for(const scope of ['vet','tas']){
          if(key===VET&&scope==='tas'||key===TAS&&scope==='vet')continue;
          if(isEditing(scope))continue;
          if(!read(scope).blocked&&canonical(operational(key,browserStorage().getItem(key),scope))===canonical(operational(key,next,scope)))continue;
          root.dispatchEvent(new CustomEvent('wwhs:team-write-blocked',{detail:{message:reason(scope),scope}}));return false;
        }
        return true;
      }catch{}
    }
  ...
  root.WWHS_TEAM_SESSION=Object.freeze({KEY,JOURNAL,scopeKey,read,isEditing,isTeamItem,allowWrite,reason});
