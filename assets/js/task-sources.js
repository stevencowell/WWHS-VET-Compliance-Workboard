(function(root){
  ...
  function refsFor(wing,task,extra=[]){
    ...
    if(wing.toLowerCase()==='tas' && !refs.some(ref=>safe(ref.url))){
      ...
      refs.push(...(task.systemIds||[]).map(id=>root.HT_TAS_WORKBOARD?.systems.find(system=>system.id===id)).filter(Boolean).map(system=>({id:system.id,title:system.label,url:safe(system.url),locator:'Working location for this local review control',accessNote:system.note||''})));
    }
  ...
  function panel(wing,task,extra=[]){
    ...
    const label=future?'2027 planning task':row?.provenanceStatus==='inferred-local-control'?'Local workboard control':row?.provenanceStatus==='specific-citation'?'Source passage located':'Exact requirement still to confirm';
    const basis=row?.sourceQuote && !future?`<div class="task-source-quote"><strong>Original source wording</strong><blockquote>${esc(row.sourceQuote)}</blockquote></div>`:row?.sourceBasis?`<p class="task-source-basis"><strong>${future?'Background to this task':'Source summary'}:</strong> ${esc(row.sourceBasis)}</p>`:`<p>This is a workboard planning control. The sources below support the work; an exact requirement for this checklist has not been established.</p>`;
    const auditUrl=`${wing.toLowerCase()==='tas'?'../':'./'}task-sources/?wing=${wing.toLowerCase()}${row?'&task='+encodeURIComponent(row.id):'#sources'}`;
    return `<section class="task-source-panel" aria-label="Task sources" data-source-task="${esc(task?.id)}"><div class="task-source-heading"><h3>Task sources</h3><span>${esc(label)}</span></div>${basis}${future?'<p class="task-source-year">The 2026 documents are background only. Confirm the current 2027 guide and deadlines before acting.</p>':''}<ul class="task-source-links">${primary.map(referenceHtml).join('')}</ul>${remainder.length?`<details class="task-source-more"><summary>More sources and access routes (${remainder.length})</summary><ul class="task-source-links">${remainder.map(referenceHtml).join('')}</ul></details>`:''}${!refs.length?'<p>No direct document has been identified for this control.</p>':''}<p class="task-source-checklist-note">The checklist below is practical workboard guidance, not a quotation from the source.</p><a class="task-source-audit" href="${auditUrl}" target="_blank" rel="noopener noreferrer">View this task’s source mapping and gaps ↗</a></section>`;
  }
